...
import SolarLunarEclipseFinder from "@/components/tools/SolarLunarEclipseFinder";
import RelatedTools from "@/components/RelatedTools";
import CopyLinkButton from "@/components/CopyLinkButton";

const TOOL_TITLE = "Next Solar & Lunar Eclipse Visibility Explorer";
const TOOL_SLUG = "solar-lunar-eclipse-finder";
...
const TOOL_META_DESCRIPTION =
  "Calculate and explore upcoming solar and lunar eclipses, paths of totality, magnitude, Saros cycles, and key visibility regions across the globe.";

export async function generateMetadata(): Promise<Metadata> {
  const canonicalUrl = `https://twistertools.com/tools/date-tools/${TOOL_SLUG}`;
  ...
  const ogImageUrl = fs.existsSync(webpPath)
    ? `https://twistertools.com/images/tools/date-tools/${TOOL_SLUG}.webp`
    : fs.existsSync(jpgPath)
      ? `https://twistertools.com/images/tools/date-tools/${TOOL_SLUG}.jpg`
  ...
  return {
    ...
    keywords: [
      ...
      "lunar eclipse tracker",
      "visibility explorer",
      "totality path calculator",
      "saros cycle database",
      "blood moon schedule",
...
export default function SolarLunarEclipseFinderPage() {
  const toolUrl = `https://twistertools.com/tools/date-tools/${TOOL_SLUG}`;

  return (
    <div className="min-h-screen bg-background">
      {/* Page Header Strip - Standard Light/Dark responsive header style matching other tools */}
      <div className="border-b border-slate-200 bg-linear-to-r from-indigo-50/80 via-white to-slate-50/70 dark:border-slate-700 dark:from-indigo-950/40 dark:via-slate-950 dark:to-slate-900/50">
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="flex items-start gap-4">
              <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-indigo-50/70 shadow-sm dark:bg-slate-800">
                <Sun className="h-8 w-8 text-indigo-600 dark:text-indigo-400" />
              </div>
