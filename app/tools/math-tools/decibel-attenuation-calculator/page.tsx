...
const DecibelAttenuationCalculator = dynamic(() => import("../../../../components/tools/DecibelAttenuationCalculator"));

const TOOL_TITLE = "Sound Decibel (dB) Distance & Attenuation Calculator";
const TOOL_SLUG = "decibel-attenuation-calculator";
const TOOL_DESCRIPTION = "Calculate sound pressure level (dB SPL) drop over distance using the Inverse Square Law, line source models, atmospheric air absorption, and OSHA safety thresholds.";
const TOOL_META_DESCRIPTION = "Professional acoustic distance attenuation calculator. Computes sound pressure level (dB SPL), sound intensity, atmospheric absorption loss, and OSHA noise safety limits based on the inverse square law.";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [
      "decibel attenuation calculator",
      "distance attenuation calculator",
      "sound pressure level drop",
      "inverse square law calculator",
      "line source sound attenuation",
      "atmospheric absorption sound",
...
export default function DecibelAttenuationCalculatorPage() {
  return (
    <div className="min-h-screen bg-background dark:bg-slate-950 pb-0 text-slate-800 dark:text-slate-200">
      {/* Page Header Strip */}
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="flex items-start gap-3 sm:gap-4">
              <div className="w-10 h-10 sm:w-14 sm:h-14 p-1 sm:p-2 rounded-xl sm:rounded-2xl shrink-0 flex items-center justify-center bg-indigo-50/70 dark:bg-slate-800 shadow-sm">
                <Volume2 className="w-7 h-7 sm:w-10 sm:h-10 text-indigo-600 dark:text-indigo-400" />
              </div>
      ...
      <main className="max-w-7xl mx-auto px-4 pt-8 pb-4">
        <div className="max-w-6xl mx-auto space-y-8">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              ...
              <div className="flex items-center gap-2 flex-shrink-0">
                ...
                <div className="relative group">
                  <a
                    href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://www.twistertools.com/tools/math-tools/${TOOL_SLUG}`)}`}
                    target="_blank"
                ...
                <div className="relative group">
                  <a
                    href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(`https://www.twistertools.com/tools/math-tools/${TOOL_SLUG}`)}&text=${encodeURIComponent(`Check out this free ${TOOL_TITLE} tool!`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                ...
                <div className="relative group">
                  <a
                    href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://www.twistertools.com/tools/math-tools/${TOOL_SLUG}`)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                ...
                {/* Copy URL */}
                <div className="relative group">
                  <CopyLinkButton url={`https://www.twistertools.com/tools/math-tools/${TOOL_SLUG}`} />
                  <span className="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-md bg-slate-900 dark:bg-slate-700 px-2 py-1 text-[11px] font-medium text-white opacity-0 group-hover:opacity-100 transition-opacity duration-150 shadow-lg z-10">
                    Copy URL
                    <span className="absolute top-full left-1/2 -translate-x-1/2 border-4 border-transparent border-t-slate-900 dark:border-t-slate-700" />
                  </span>
          ...
          {/* Related Tools */}
          <RelatedTools currentSlug={TOOL_SLUG} currentCategory="math-tools" />
        </div>
      </main>
    </div>
  );
