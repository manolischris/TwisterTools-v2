...
import TournamentBracketGenerator from "@/components/tools/TournamentBracketGenerator";
import RelatedTools from "@/components/RelatedTools";
import CopyLinkButton from "@/components/CopyLinkButton";

const TOOL_TITLE = "Tournament Bracket & Seeding Generator";
const TOOL_SLUG = "tournament-bracket-generator";
...
const TOOL_META_DESCRIPTION =
  "Generate customizable single-elimination and tournament brackets with cryptographic random shuffling, ranked seeding, automatic bye calculations, and interactive match score tracking.";

export async function generateMetadata(): Promise<Metadata> {
  const canonicalUrl = `https://www.twistertools.com/tools/random-tools/${TOOL_SLUG}`;
  ...
  const ogImageUrl = fs.existsSync(webpPath)
    ? `https://www.twistertools.com/images/tools/random-tools/${TOOL_SLUG}.webp`
    : fs.existsSync(jpgPath)
      ? `https://www.twistertools.com/images/tools/random-tools/${TOOL_SLUG}.jpg`
      : "https://www.twistertools.com/images/og-default.jpg";

  return {
    title: "Tournament Bracket & Seeding Generator | Free Single & Double Elimination Tool",
    ...
    openGraph: {
      title: "Tournament Bracket & Seeding Generator | Free Single & Double Elimination Tool | TwisterTools",
      description: TOOL_META_DESCRIPTION,
      url: canonicalUrl,
...
export default function TournamentBracketGeneratorPage() {
  const toolUrl = `https://www.twistertools.com/tools/random-tools/${TOOL_SLUG}`;
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 md:px-8 py-2.5 sm:py-3 md:py-3.5">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="scrollbar-none mb-1.5 flex flex-wrap items-center gap-1 overflow-x-auto whitespace-nowrap text-xs text-slate-500 dark:text-slate-400">
              ...
              <Link className="max-w-32.5 truncate font-medium transition-colors hover:text-indigo-600 dark:hover:text-indigo-400 sm:max-w-50 md:max-w-none" href="/tools/random-tools">
            ...
            <div className="flex items-start gap-3 sm:gap-4">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-indigo-200/80 bg-gradient-to-br from-indigo-50 to-white p-1 shadow-xs shadow-indigo-100/60 dark:border-indigo-900/50 dark:from-slate-800 dark:to-slate-900 sm:h-14 sm:w-14 sm:rounded-2xl sm:p-2">
                <Trophy className="h-6 w-6 text-indigo-600 dark:text-indigo-400 sm:h-9 sm:w-9" />
              </div>
      ...
      <main className="mx-auto max-w-7xl px-4 py-4 sm:px-6 md:px-8 md:py-6">
        <div className="mx-auto max-w-6xl space-y-8">
          ...
          <RelatedTools currentSlug={TOOL_SLUG} currentCategory="random-tools" />
        </div>
      </main>
    </div>
  );
