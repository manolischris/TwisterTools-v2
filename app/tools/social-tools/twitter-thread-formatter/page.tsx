...
import TwitterThreadFormatter from "@/components/tools/TwitterThreadFormatter";
import RelatedTools from "@/components/RelatedTools";
import CopyLinkButton from "@/components/CopyLinkButton";

const TOOL_TITLE = "Twitter Thread Splitter & Sequence Formatter";
const TOOL_SLUG = "twitter-thread-formatter";
const TOOL_DESCRIPTION =
  "Split articles, essays, and long drafts into character-accurate, numbered Twitter/X threads with smart sentence boundary detection.";
const TOOL_META_TITLE =
  "Twitter Thread Splitter & Sequence Formatter";
...
export async function generateMetadata(): Promise<Metadata> {
  const canonicalUrl = "https://twistertools.com/tools/social-tools/twitter-thread-formatter";
...
export default function TwitterThreadFormatterPage() {
  const toolUrl = "https://twistertools.com/tools/social-tools/twitter-thread-formatter";
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="flex items-start gap-3 sm:gap-4">
              <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-indigo-200/80 bg-gradient-to-br from-indigo-50 to-white p-1 shadow-xs shadow-indigo-100/60 dark:border-indigo-900/50 dark:from-slate-800 dark:to-slate-900 sm:h-14 sm:w-14 sm:rounded-2xl sm:p-2">
                <TwitterIcon className="h-6 w-6 text-indigo-600 dark:text-indigo-400 sm:h-9 sm:w-9" />
              </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-slate-50 to-white p-5 shadow-sm dark:border-slate-700 dark:from-slate-900 dark:to-slate-800">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                <div className="group relative">
                  <a
                    href={`https://twitter.com/intent/tweet?url=${encodeURIComponent(toolUrl)}&text=${encodeURIComponent("Check out this free Twitter Thread Splitter & Sequence Formatter tool!")}`}
                    target="_blank"
                    rel="noopener noreferrer"
