...
// Helper to convert WebP and other unsupported formats to PNG via canvas
const convertImageToPng = (imgUrl: string): Promise<ArrayBuffer> => {
            ...
            canvas.toBlob((blob) => {
                ...
                reader.onloadend = () => {
                    if (reader.result instanceof ArrayBuffer) {
                        ...
                        reject(new Error("Failed to read converted blob as ArrayBuffer"));
                    }
                };
        ...
        try {
            ...
            for (const item of images) {
                ...
                if (fileType === "image/png" || fileName.endsWith(".png")) {
                    ...
                    // Convert WebP / other formats dynamically to PNG
                    try {
                        const pngBuffer = await convertImageToPng(item.previewUrl);
                        pdfImage = await pdfDoc.embedPng(pngBuffer);
                    } catch (err) {
                        console.error("Canvas conversion failed, falling back to direct array buffer read:", err);
                        const arrayBuffer = await item.file.arrayBuffer();
                        pdfImage = await pdfDoc.embedJpg(arrayBuffer);
                    }
                }
                    ...
                    <div className="bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
                        ...
                        <div className="bg-gradient-to-r from-slate-800 to-indigo-600 px-5 py-2.5 flex items-center justify-between text-white">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <h1 className="text-base font-bold leading-tight">1. Source Images</h1>
            ...
            <section className="space-y-8 mt-12">
                        ...
                        <table className="w-full border-collapse text-left">
                            ...
                            <tbody className="divide-y divide-slate-200 text-xs md:text-sm text-slate-700">
                                ...
                                <tr className="bg-white">
                                    ...
                                    <td className="px-4 py-3">A4 / Letter / Auto Fit</td>
                                    <td className="px-4 py-3">0px, 20px, 40px</td>
                                    <td className="px-4 py-3 font-mono text-xs text-indigo-600">Canvas to PNG Transcoder</td>
                                    <td className="px-4 py-3 font-semibold text-emerald-600">Lossless PNG Conversion</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-8 shadow-sm space-y-6">
                    <h2 className="text-2xl font-bold text-slate-900 flex items-center gap-3">
                        ...
                        <span>How to Convert Images to PDF</span>
                    </h2>
            ...
            <script
                ...
                dangerouslySetInnerHTML={{
                    __html: JSON.stringify({
                        ...
                        "@type": "WebApplication",
                        name: "Image to PDF Universal Converter",
